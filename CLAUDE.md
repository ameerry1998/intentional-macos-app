...
12. **Bedtime trap conditions (April 28, 2026 hardening).** First-pass cross-device bedtime had no escape valve — user could lock at 11pm and be stuck until wake alarm. Plus a single NFC scan dismissing the wake alarm could double-fire and start Deep Work, plus the iOS Shield could survive process death with `activeStore` nil so the app couldn't deactivate it from the UI. Fixes shipped: backend `/bedtime/unlock-{request,verify,status}` endpoints (mirror `/unlock/*`), iPhone takeover overlay + partner-unlock flow + disambig modal for alarm-dismiss puck, Mac BedtimeEnforcer reads `releasedUntil` ahead of schedule + polls `/bedtime/unlock-status` every 5s while locked, BlockingService restoreShieldStateIfActive reconstructs `activeStore`. See `docs/cross-repo-bedtime-lockdown-2026-04-28.md` and `docs/superpowers/plans/2026-04-28-bedtime-lockdown-final.md`.
