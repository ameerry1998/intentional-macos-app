...
| **D15** | **Cool-down dialog uses the WARM / personified tone**, not clinical. Headline pattern: *"Tomorrow you, not right-now you, gets the change."* Body explains the cool-down length and reason (*"You picked Strict because it's where the work happens. The cool-down is the part that protects that decision."*). Buttons: `[Keep Strict]` `[Start cool-down]` and `[Ask partner]` for Strict step-down. | Matches the ICP framing of "future-self gets the change, not right-now-self." Clinical tone tested in design felt cold. |
| **D16** | **iOS Wake/Bedtime banners live INSIDE the schedule card** — they are NOT page-level sticky chrome between the status bar and tab bar. The handoff design currently shows them as page-level horizon bands wrapping the entire Schedule tab content (Today header + Day/Week toggle + timeline). That's wrong — they must be **inside the timeline card itself** (top edge + bottom edge), with the same border-radius and padded margins as the rest of the card. Mac already does this correctly (slabs inside the timeline border-radius). | Page-level chrome makes Wake/Bedtime feel like global app chrome rather than schedule content. They're schedule artifacts; they belong in the schedule card. Match Mac's pattern. |
| **D17** | **The handoff design files in `~/Downloads/handoff-schedules/` are the source of truth for all visual choices** (typography, color, spacing, micro-interactions, iconography). Implementation must mirror the design closely. Where the design and this spec conflict, **the spec wins on PRODUCT decisions (e.g. D10 strictness-only-on-Intention, D12 one-off-block, D16 inside-schedule-card)**, but the **design wins on VISUAL choices** (colors, fonts, exact pixel offsets, animation timing). | The design supplements the spec; both are required reading for any executor. |
