...
- R4 enforcement unification: done — EnforcementResolver (one precedence: per-goal allow > ✅ > 🚫/⏳gate > goal blocklist > default) feeds both FocusMonitor and WebsiteBlocker; allow-lists now protect SITES (verified live before/after); session-start + sweep honor rule enabled/snoozes; Strict Mode lock gates real-store mutations server-side-of-bridge (verified refusal with live lock). ⏳ outside sessions = TODO(R5). Follow-up holes filed: SAVE_STRICT_MODE_LOCKS itself ungated; legacy UPDATE_BLOCK_RULE ungated — **both closed 2026-06-11** (lock-map gate + legacy-path gate, live-verified refusal + pass on-machine; see Key invariants).
...
- Asymmetric locking: tightening always free; loosening (delete/disable 🚫, demote, raise base/cap, lower rate) partner-gated when Strict Mode on — `isLooseningAction` in dashboard.html, mirrored server-side-of-bridge by `blockRulesLockEngaged()` in MainWindow.swift.
- The lock map itself is lock-protected (2026-06-11): `SAVE_STRICT_MODE_LOCKS` refuses disabling any currently-enabled lock item (true→false) while Strict Mode is on without an active partner-unlock window (`{success:false, reason:"strict_mode_locked"}` + checkbox revert + unlock-sheet flow via `_strictModeLocksResult`). Enabling locks always passes; with Strict Mode off everything passes. Closes the R4 follow-up bypass (uncheck `block_rules`, then loosen freely). Legacy `UPDATE_BLOCK_RULE`/`UPDATE_BLOCKING_PROFILE` loosening edits are gated the same way.
