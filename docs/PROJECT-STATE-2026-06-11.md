...
3. ~~Onboarding redesign~~ DONE + VM-VERIFIED (2026-06-11 night): 11-screen value-before-commitment flow shipped (commits 253f93c/97ab306/a143c77/e5c46f9), Opal-cloned copy, seeds real Rules, first session starts inside onboarding, partner post-win. Fresh-account walk in VM: goal created + session started + dashboard lands mid-session with goal visible ("5 rules active / Focusing"). Doc: docs/features/onboarding.md. **Bonus launch-blocker found+fixed (5421c95): backend fractional-second timestamps broke ALL intention/monthly-goal decode** — every fresh account had goal-creation/session-start dead; existing installs silently ran off stale caches. ISO8601Tolerant strategy now used by intentionsJSONDecoder (12 sites) + store caches. KNOWN first-run jank: close-the-noise sweep fires over onboarding screen 11 when the first session starts — needs suppress-during-onboarding.
4. **Cold fresh-install test in VM** — real notarized installer, fresh signup → first session, end to end. Highest-risk unknown (never tested by a non-Ameer user). NOTE: use a CLEAN VM clone — the current intentional-test VM has ad-hoc-signing keychain pollution (every re-signed dev build re-prompts keychain ACLs; notarized PKG won't have this).
5. ~~Notarized PKG/DMG build~~ BUILT 2026-06-11 night (exit 0, notarized): /tmp/intentional-pkg-build/Intentional-1.0.pkg (314MB). Still needs the clean-machine verify (item 4).
